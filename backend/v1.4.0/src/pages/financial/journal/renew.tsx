...
import {
    paymentMeansListApi,
    abstractsApi,
    generalJournalListApi,getToken,
...
const JournalRenew = (props,ref) => {
    ...
    const [generalJournal,setGeneralJournal] = useState([{index:0, flag: '1', source: null, amount: 0, remarks: ''}])
    ...
    const getData = async (id) => {
        ...
        setLoading(true);
        // 发异步ajax请求, 获取数据
        const {err,result} = await generalJournalListApi(para);
        if (err){
            console.error('获取财政列表数据异常:',err)
            setLoading(false)
            return
        }
    ...
    const initPaymentMeans = async () => {
        // 发异步ajax请求, 获取数据
        const {err,result} = await paymentMeansListApi();
        if (err){
            console.error('获取支付类别异常:',err)
            return
        }
        const {msg, code, data} = result;
        if (code === 0) {
    ...
    const initAbstracts = async () => {
        // 发异步ajax请求, 获取数据
        const {err,result} = await abstractsApi();
        if (err){
            console.error('获取交易摘要异常:',err)
            return
        }
        const {msg, code, data} = result;
        if (code === 0) {
    ...
    const initMonetaryData = async () => {
        // 发异步ajax请求, 获取数据
        const {err, result} = await monetaryListApi();
        if (err){
            console.error('获取币种异常:',err)
            return
        }
        const {msg, code, data} = result
        if (code === 0) {
    ...
    const deleteTransactionItem = async (id,_journal,_generalJourna) => {
        setLoading(true);
        const {err, result} = await deleteGeneralJournalApi(id);
        if (err){
            console.error('删除财政明细数据异常:',err)
            setLoading(false)
            return
        }
    ...
    const addTransactionItem = async (value) => {
        ...
        setLoading(true);
        const {err, result} = await addGeneralJournalApi(para);
        if (err){
            console.error('添加财政明细数据异常:',err)
            setLoading(false)
            return
        }
        const {msg, code} = result
    ...
    const renewTransactionItem = async (value) => {
        ...
        setLoading(true);
        const {err, result} = await updateGeneralJournalApi(para);
        if (err){
            console.error('更新财政明细数据异常:',err)
            setLoading(false)
            return
        }
        const {msg, code} = result
    ...
    const handleEdit = async () => {
        ...
        const oldDate = dayjs(journal.archive_date, format);
        const newDate = dayjs(latestJournal.archive_date, format);
        if ((!oldDate.isSame(newDate)) || (journal.means_id !== latestJournal.means_id) || (journal.abstract_id !== latestJournal.abstract_id) || (journal.monetary_id !== latestJournal.monetary_id)|| (journal.remarks !== latestJournal.remarks)) {
            ...
            const {err, result} = await updateJournalApi(param);
            if (err){
                console.error('更新财政数据异常:',err)
            ...
            <section className="journal-details">
                ...
                <Row className="detail-archive-date">
                    <Col span={6} offset={18}>
                        ...
                        value={!latestJournal || !latestJournal.archive_date ? null : dayjs(latestJournal.archive_date)}
                ...
                <Row>
                    <Col span={24}>
                        <Table size="small" className='detail-grid' rowKey="index" bordered pagination={false}
