...
const Journal = () => {
            ...
            render: (value, row) => (
                <div>
                    ...
                    <Button type="primary" size="small" onClick={() => openEditModal(row)} shape="circle"
                            icon={<EditOutlined/>}/>
                    &nbsp;
                    <Button type="primary" danger="true" size="small" onClick={() => handleDelete(row)} shape="circle"
                            icon={<DeleteOutlined/>}/>
                </div>
            ),
        },
    ...
    const getData = async (_filters = filters,_pagination= pagination) => {
        ...
        setLoading(true);
        // 发异步ajax请求, 获取数据
        const {err,result} = await getTransactionList(para);
        if (err){
            setLoading(false)
    ...
    const handleDelete = (item) => {
        Modal.confirm({
            ...
            onOk: async () => {
                // 在发请求前, 显示loading
                setLoading(true);
                const {err,result} = await deleteJournalApi(item.id);
                if (err){
                    console.error('删除流水申报:',err)
                    setLoading(false)
                    return
                }
