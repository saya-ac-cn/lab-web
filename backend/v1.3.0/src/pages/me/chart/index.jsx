...
class Chart extends Component {
  ...
  loadAccountGrowthRate = async () => {
    ...
    // 发异步ajax请求, 获取数据
    const {msg, code, data} = await getAccountGrowthRate({archive_date:(accountGrowthRate.tradeDate).format('YYYY-MM-DD')});
  ...
  loadIncomePercentage = async () => {
    ...
    // 发异步ajax请求, 获取数据
    const {msg, code, data} = await getIncomePercentage({archive_date:(incomePercentage.tradeDate).format('YYYY-MM-DD')});
  ...
  loadActivityRate= async () => {
    ...
    // 发异步ajax请求, 获取数据
    const {msg, code, data} = await getActivityRate({archive_date:(activityRate.tradeDate).format('YYYY-MM-DD')});
  ...
  loadNewsRate = async () => {
    ...
    // 发异步ajax请求, 获取数据
    const {msg, code, data} = await getNewsRate({archive_date:(newsRate.tradeDate).format('YYYY-MM-DD')});
  ...
  loadOrderByAmount = async () => {
    ...
    // 发异步ajax请求, 获取数据
    const {msg, code, data} = await getOrderByAmount({archive_date:(financial.tradeDate).format('YYYY-MM-DD')});
  ...
  loadPreSixMonthBill = async () => {
    ...
    // 发异步ajax请求, 获取数据
    const {msg, code, data} = await getPreSixMonthBill({archive_date:(financial.tradeDate).format('YYYY-MM-DD')});
  ...
  loadCountAndWordCloud= async () => {
    ...
    if (code === 0) {
      // 发异步ajax请求, 获取数据
      countAndWordCloud.serverRoseData = data.rose_data;
      countAndWordCloud.serverWordCloudData = data.word_cloud;
      _this.setState({countAndWordCloud});
    }else {
      _this.setState({countAndWordCloud});
      openNotificationWithIcon("error", "错误提示", msg);
