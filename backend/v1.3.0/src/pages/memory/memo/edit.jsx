...
import {createMemoApi, updateMemoApi} from "../../../api";
import {openNotificationWithIcon} from "@/utils/window";
/*
 * 文件名：edit.jsx
 * 作者：shmily
 * 创建日期：2019-10-05 - 21:50
 * 描述：便利贴表单
 */
...
class EditMemo extends Component {
    ...
    handleAddMemo = (value) => {
        ...
        Modal.confirm({
            title: '您确定创建该便利贴?',
            onOk: async () => {
                const {msg, code} = await createMemoApi(value);
                if (code === 0) {
    ...
    handleRenewMemo = (value) => {
        ...
        Modal.confirm({
            title: '您确定要保存此次修改结果?',
            onOk: async () => {
                const {msg, code} = await updateMemoApi(value);
                if (code === 0) {
    ...
    render() {
        ...
        return (
            <Modal title={!memo || !memo.id ? '添加便利贴':'编辑便利贴'} open={visibleModal} maskClosable={false} okText='保存' onOk={this.handleSubmit} onCancel={this.handleCancel}>
