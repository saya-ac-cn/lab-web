...
class NewsInfo extends Component {
    ...
    initDatas = async () => {
        ...
        const {msg, code, data} = await queryNewsInfo(this.state.id);
        // 在请求完成后, 隐藏loading
        this.setState({listLoading: false});
        if (code === 0) {
            const thisNews = data.now;
            thisData = data
                                                    ...
                                                    <div className="news-footer">
                                                        <ul>
                                                            <li className="pre-li">
                                                                <span>上一篇</span>
                                                                {
                                                                    !(isEmptyObject(datas.pre)) ? <a href={`/public/news/${datas.pre.id}`}>{datas.pre.topic}</a> : <a href="#base-content">已是第一篇了</a>
                                                                }
                                                            </li>
                                                            <li className="next-li">
                                                                <span>下一篇</span>
                                                                {
                                                                    !(isEmptyObject(datas.next)) ? <a href={`/public/news/${datas.next.id}`}>{datas.next.topic}</a> :
